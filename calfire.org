...
To estimate \ac{GHG} emissions from biomass combustion, we estimates pile, wildfire, and
...
Emissions Calculator. The following ratios are applied to CARB-reported PM emissions to estimate \ac{GHG} emissions (Table ref:tab:pfe_calc).
...
- To estimate \ac{GHG} emissions from *wildfire* and *slash burning*, we In addition, the
...
ratio of \ac{GHG} to PM using the [[http://www.fs.fed.us/pnw/fera/research/smoke/consume/index.shtml][USFS CONSUME]] model. This analysis does however show that a baseline of
