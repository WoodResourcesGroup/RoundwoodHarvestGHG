...
aggregate annual climate-forcing emissions ref:fig:burn_diag. The GHG inventory captures
...
forcing properties ref:tab:bc_gwp. The citet:CaliforniaAirResourcesBoard2015,CaliforniaAirResourcesBoard2016
CAP report captures SLCP emissions from wildfire
(src_python{return(0.66*122)} MMTCO2e) and prescribed fire
(src_python{return(0.03*122)} MMTCO2e) from which black carbon emissions may be estimated., //but no reference in the
SLCP Strategy is made to the source of these estimates//. When viewed in aggregate, a comprehensive reporting of total climate impact from anthropogenic burning may be estimated. 
...
The [[http://www.arb.ca.gov/ei/emissiondata.htm][2015 Criteria air pollutant (CAP)
emissions estimates]] published by the California Air Resources Board reports on particulate matter (PM 2.5) of which black carbon is a component. CARB reports PM 2.5 emissions in tons/day. Annual emissions as
...
of the Total Carbon (TC) component of PM 2.5. Using estimates of
the ratio of smoldering to flaming combustion for hand/machine piled
burns, prescribed natural fire and wildfire from citet:Ward1989, Black Carbon emissions
can be estimated from PM 2.5 emissions. Thus BC may be calculated from PM
...
#+END_SRC
#+CAPTION: Peter, I feel like there is too much going on in this table. Factors used for calculating Black Carbon (BC) emissions from the three primary combustion sources. BC is a fraction of Total Carbon (TC) which is a fraction of total PM 2.5. Coefficients of variation (C_v) are reported here as well.
#+RESULTS: tab:bc_pm
+------------+----------------+---------------------+--------------------+--------------------+---------------------+---------------------+
...
3. Estimate annual BC emissions reg:tab:carb_bc based on probability distributions
   defined in step 2.
...
PM2.5 to CO2 and CH4 ref:tab:pfe_calc from the Piled Fuels Emissions Calculator. The following ratios are then used to estimate GHG emissionsn from CARB-reported PM emissions.
...
#+CAPTION: Ratios of Biomass to GHG emissions from the Piled Fuels Emissions Calculator.
...
#+END_SRC
#+CAPTION: Annual GHG Emissions estimated from CARB GHG emissions inventory
...
To arrive at an estimate of total CO2 equivalent emissions in 2015 from burning of forest
...
#+END_SRC
#+CAPTION: Estimated average annual CO2 equivilant emissions by source and emission type.
...
landscaping, and consumer products. During the manufacture of these products, this wood is fractionated through a multi-stage process of harvesting, processing, and use into several ??carbon pool?? categories (below) for which the time horizon of carbon return to the atmosphere varies widely. 
