...
To provide a rough estimate of the fate of annually harvested roundwood material, we estimate volumes of wood biomass residing in Logging, Processing, and Construction 
...
For logging residuals not used in bioenergy, emmisions are produced
...
management residuals, then compare with total biomass volume of residues as reported by the Timber Products Output (TPO) database. 
C02e emissions are then independently derived for both conversion streams of logging residuals. 
/// To establish the fraction of logging residue that is left to decompose, I 
utilize the CARB estimate of annual PM2.5 emissions produced from
