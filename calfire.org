...
This analysis provides insight into opportunities to more effectively utilize woody biomass residuals from current forest management activities based on available historical data. Estimates are based on empirical data and reflect past forest management activities. It is *critical* to note that the empirical data used in this analysis reflect point-in-time measures that are
affected by a dynamic system of climate, growth, and mortality in
forests as well as macroeconomic and policy forces. To effectively
...
Two approaches are deployed here to estimate aggregate CO2 equicalent emissions from anthropogenic biomass burning in California. The first is based on reported emissions from regional air quality offices. The second is derived from timber harvest activities tracked through tax records.
...
The California Board of Equalization cite:CaliforniaStateBoardofEqualization2015 tracks timber harvests by year for public and private lands. Others cite:Mciver2012 report the fate of harvested wood that is taken to a mill or bioenergy plant.  We can use the timber harvest data in combination with logging removal factors from cite:Morgan and estimates of in-woods biomass consumed in energy production to ascertain an estimate of the volume of logging residuals left in the woods. 
...
reported by CARB are shown in Table ref:arb_pm_ann.
...
#+END_SRC
#+CAPTION: Emissions of PM 2.5 in 2015 as reported by CARB. ALL VEGETATION: Wildfire, FOREST MANAGEMENT: Pile burning, WILDLAND FIRE USE (WFU): Prescribed burn.
#+RESULTS: arb_pm_ann
| Source                  | PM 2.5 (t)        |
|-------------------------+-------------------|
...
Black Carbon emissions can be estimated from PM 2.5 emissions if the ratio of smoldering to
flaming combustion is known. citet:Ward1989,Jenk1996 provide estimates of the ratio of smoldering to flaming combustion for hand/machine piled burns, prescribed natural fire and wildfire. BC is a fraction of the Total Carbon (TC) component of PM 2.5. Thus BC may be calculated from PM
...
Based on citet:Ward1989 and citet:Jenk1996, ratios in Table ref:tab:bc_pm are used herein.
...
--    tc_f_cv as 'TC_f^{Cv} t^{-1} PM',
    ...
    "Unnamed 1" as 'BC_s t^{-1} PM 2.5',
--    tc_s_cv as 'TC_s^{Cv} t^{-1} PM',
    ec_s_cv as 'BC_s^{Cv} t^{-1} TC'
from ec_ratios;
...
#+RESULTS: tab:bc_pm
| Source     | BC_f t^{-1} PM | BC_f^{Cv} t^{-1} TC | BC_s t^{-1} PM 2.5 | BC_s^{Cv} t^{-1} TC |
|------------+----------------+---------------------+--------------------+---------------------|
| Pile Burn  |       0.046904 |                0.45 |            0.01624 |                0.49 |
| Prescribed |     0.08016309 |              0.5833 |           0.020944 |                0.29 |
| Wildfire   |     0.05870124 |              0.4467 |          0.0228641 |               0.338 |



To arrive at a rough estimate of BC emissions based on PM2.5 we determine the amount of PM2.5 produced in the flaming and smoldering phases of combustion for each type (piles, prescribed, wildfire). Ratios from citet:Ward1989 Table 5 are used. We then estimate annual BC emissions using the ratios applied to the estimated volume of burned biomass.
...
** Estimating total emissions from combustion of forest management residuals
...
* Estimating emissions impact from harvested wood management
